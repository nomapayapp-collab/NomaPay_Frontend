    ...
    >
        ...
        <div className="grid grid-cols-1 xl:h-33.5 xl:grid-cols-[1.1fr_3fr]">
          <div className="flex flex-col justify-center p-5 xl:border-r xl:border-white/10 xl:px-8 xl:py-4">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
              {!loading &&
                (balancePct === null ? (
                  <span className="text-slate-500 dark:text-slate-300">Sin datos de la semana pasada</span>
                ) : (
                  <>
                    <span
                      className={`font-bold ${
                        formatPct(balancePct).positive ? "text-cyan-500 dark:text-cyan-300" : "text-magenta-500"
                      }`}
                    >
                      {formatPct(balancePct).positive ? "↗" : "↘"} {formatPct(balancePct).label}
                    </span>
                    <span className="text-slate-500 dark:text-slate-300">vs semana pasada</span>
                  </>
                ))}
            </div>
          </div>
      ...
      >
        ...
        <Card variant="elevated" className="min-h-0 min-w-0 p-5 xl:overflow-hidden xl:p-6">
          ...
          {loading ? (
            <div className="mt-5 flex h-85 items-center justify-center text-sm text-slate-500 dark:text-slate-400 xl:h-[calc(100%-44px)]">
              Cargando...
            </div>
          ) : error ? (
            ...
            <div className="mt-5 flex h-85 items-center justify-center text-center text-sm text-slate-500 dark:text-slate-400 xl:h-[calc(100%-44px)]">
        ...
        <div className="flex min-h-0 flex-col gap-4">
          ...
          <div className="shrink-0 rounded-xl border border-violet-400/50 bg-violet-500/10 px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900 dark:text-white">Resumen semanal por correo</p>
        ...
        <Card variant="elevated" className="min-h-0 p-4 xl:overflow-hidden">
          <h2 className="text-sm font-bold text-slate-900 dark:text-white">Desglose por tipo</h2>
        ...
        {/* Comparación */}
        <Card variant="elevated" className="min-h-0 p-4 xl:overflow-hidden">
          <h2 className="text-sm font-bold text-slate-900 dark:text-white">Comparado con la semana pasada</h2>
    ...
    <div className="flex h-full min-w-0 flex-1 flex-col justify-end">
      ...
      <div
        className={`mt-3 border-t border-slate-300 pt-2 text-center text-xs font-semibold dark:border-[#30385d] ${
          isToday ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"
        }`}
      >
        {day.label}
      </div>
    ...
    <div>
      <div className="flex items-center justify-between gap-3">
        <p className="min-w-0 text-xs">
          ...
          <span className="ml-1 text-slate-500 dark:text-slate-300">· {detail}</span>
        </p>
