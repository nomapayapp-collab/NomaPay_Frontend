...
export default function Wallet() {
  ...
  const movementsMaxHeight = isDesktop ? leftColumnHeight : null;
        ...
        <>
          ...
          <div className="hidden lg:flex lg:flex-col lg:gap-6">
            ...
            <div className="lg:grid lg:grid-cols-3 lg:gap-6 lg:items-start">
              <div className="flex flex-col gap-4" ref={leftColumnRef}>
                {renderFavoriteCurrency()}
                {renderCargarSaldoButton()}
              </div>
              <div className="lg:col-span-2">
                <div
                  className="rounded-card border border-border-light bg-surface-light dark:border-border-dark dark:bg-surface-dark-elevated p-6 lg:flex lg:flex-col"
                  style={movementsMaxHeight != null ? { height: movementsMaxHeight } : undefined}
                >
                  <p className="card__title mb-3 lg:shrink-0 flex">Movimientos en {selected.currency.code} <IconStar className="w-3.5 h-3.5 text-amber-500" /></p>
                  {movements.length === 0 ? (
